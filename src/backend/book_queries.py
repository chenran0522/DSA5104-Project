...
def search_book_by_author_name():
    ...
    cursor.execute(f"use systemdatabase")
    cursor.execute(
        f"SELECT * FROM book join book_author on book.book_id = book_author.book_id join authors on authors.author_id = book_author.author_id WHERE authors.author_name like CONCAT('%', '{author_name}', '%')"
    )
    mysql_result = cursor.fetchall()
...
def search_book_by_watch_numbers():
    ...
    mongo_collection = mongo_db["book"]
    mongo_results = list(
        mongo_collection.find({"watch_numbers": {"$gte": watch_numbers}})
    )
    data = {"mysql_data": mysql_result, "mongo_data": mongo_results}
    response = make_response(json.dumps(data, default=json_encoder), 200)
    response.mimetype = "application/json"
...
@bp.route(
    "/find_top_watch_numbers_book", methods=["POST"]
)  # 查询top n watch_numbers book
def find_top_watch_numbers_book(limit=10):
    ...
    mongo_collection = mongo_db["book"]
    mongo_results = list(
        mongo_collection.find().sort([("watch_numbers", -1)]).limit(top)
    )
    data = {"mysql_data": mysql_result, "mongo_data": mongo_results}
    response = make_response(json.dumps(data, default=json_encoder), 200)
    response.mimetype = "application/json"
