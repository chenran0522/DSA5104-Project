...
def search_game_by_director_name():
    ...
    cursor = mydb.cursor()
    cursor.execute(f"SELECT * FROM game join Game_director on game.game_id = Game_director.game_id join Director_id on Director_id.Director_id = Game_director.Director_id WHERE Director_id.name like CONCAT('%', '{director_name}', '%')")
    mysql_result = cursor.fetchall()
...
def search_game_by_genres():
    ...
    cursor = mydb.cursor()
    cursor.execute(f"SELECT * ,genres.name AS genres_name FROM game join game_genre on game.game_id = game_genre.game_id join genres on genres.genre_id = game_genre.genre_id WHERE genres.name like CONCAT('%', '{genres}', '%')")
    mysql_result = cursor.fetchall()
...
def find_game_all():
    ...
    rating = float(request.json.get('rating', 0)) if request.json.get('rating', '') != '' else 0
