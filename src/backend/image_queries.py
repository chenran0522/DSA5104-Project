...
def search_image_fuzzy():
    ...
    cursor = mydb.cursor()
    cursor.execute(
        f"SELECT * FROM systemdatabase.image WHERE name  LIKE CONCAT('%', '{title}', '%') "
    )
    mysql_result = cursor.fetchall()
...
def search_image_by_id():
    ...
    mydb = mysql.connector.connect(
        host="127.0.0.1", user="root", port=3306, password="Wxp-0525"
    )
    cursor = mydb.cursor()
...
def find_pop_images(limit=10):
    ...
    cursor = mydb.cursor()
    cursor.execute(
        f"SELECT * FROM systemdatabase.image ORDER BY watch_numbers DESC LIMIT {num}"
    )
    mysql_result = cursor.fetchall()
    ...
    mongo_collection = mongo_db["image"]
    mongo_results = list(
        mongo_collection.find().sort([("watch_numbers", -1)]).limit(num)
    )
    data = {"mysql_data": mysql_result, "mongo_data": mongo_results}
    response = make_response(json.dumps(data, default=json_encoder), 200)
    response.mimetype = "application/json"
...
def find_top_rated_images(limit=10):
    ...
    cursor = mydb.cursor()
    cursor.execute(
        f"SELECT * FROM systemdatabase.image ORDER BY rating DESC LIMIT {num}"
    )
    mysql_result = cursor.fetchall()
...
def find_images_by_genres():
    ...
    cursor.execute(
        f"SELECT * FROM image as I natural join image_genre as IG where IG.genre_id = (SELECT genre_id from genres where name = '{genre}')"
    )
    mysql_result = cursor.fetchall()
...
def find_images_by_cast():
    ...
    cursor = mydb.cursor()
    cursor.execute(
        f"SELECT * FROM systemdatabase.image as I natural join image_cast as IC where IC.cast_id in (SELECT cast_id from cast_info where cast_name like concat ('%', '{cast}', '%'));"
    )
    mysql_result = cursor.fetchall()
...
def find_images_join():
    ...
    cursor.execute(f"use systemdatabase")
    cursor.execute(
        f"(select distinct * from image where image_id in (select image_id from image natural join image_cast natural join cast_info where cast_name like concat ('%', '{cast}', '%'))) union(select distinct * from image where image_id in (select distinct image_id from image natural join image_cast natural join cast_info where name like concat ('%', '{cast}', '%')));"
    )
    mysql_result = cursor.fetchall()
