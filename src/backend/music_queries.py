...
def search_image():
    ...
    cursor = mydb.cursor()
    cursor.execute(f"SELECT * FROM systemdatabase.music WHERE music_name = '{music_name}'")
...
def search_image_fuzzy():
    ...
    cursor = mydb.cursor()
    cursor.execute(f"SELECT * FROM music WHERE music_name  LIKE CONCAT('%', '{music_name}', '%') ")
    mysql_result = cursor.fetchall()
...
def find_images_by_genres():
    ...
    cursor = mydb.cursor()
    cursor.execute(f"SELECT * FROM music as I natural join music_genres as MG where MG.genre_id = (SELECT genre_id from genres where name = '{genre}')")
    mysql_result = cursor.fetchall()
...
def find_images_by_cast():
    ...
    cursor.execute(f"SELECT * FROM music as I natural join music_artist as MA where MA.artist_id in (SELECT artist_id from artist where artist_name like concat ('%', '{artist}', '%'));")
    mysql_result = cursor.fetchall()
...
@bp.route('/find_music', methods=['POST']) # 综合（模糊title、模糊cast、genre、高分要求、format、默认输出rating最高的10个）
def find_images():
